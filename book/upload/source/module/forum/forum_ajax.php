...
EOF;
    ...
    $cid = $_GET['cid'];
    $teachers = C::t('book_teacher')->query_by_collegeId($cid);
    $htmlSeg = '';
    foreach($teachers as $teacher) {
        $htmlSeg = $htmlSeg.'<option value="'.$teacher['id'].'">'.$teacher['number'].'->'.$teacher['name'].'</option>';
    }
    showmessage($htmlSeg, '', array('a'=>'aaa','b'=>'bbb'), array('msgtype' => 3));
}
